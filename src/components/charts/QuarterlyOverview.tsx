...
const QuarterlyOverview: React.FC<QuarterlyOverviewProps> = ({ quarterlyData }) => {
    ...
    return (
        <div className="space-y-0">
            ...
            <div className=" md:hidden md:mt-16 mt-8">
                <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide space-x-4 px-4 pb-6">
                    {quartersData.map(({ quarter, techno, trance, visibleGenre, monthRange }) => {
                        ...
                        return (
                            <div 
                                key={`quarter-${quarter}`} 
                                className="flex-shrink-0 w-[85vw] snap-start space-y-3 p-4 rounded-xl bg-gray-800/50 border border-gray-700"
                            >
                                ...
                                <div className="flex flex-col items-center space-y-2">
                                    <div className="flex items-center mx-auto max-w-xs">
                                        <span className={`text-sm font-medium ${visibleGenre === 'TECHNO' ? 'text-spotify-green' : 'text-gray-400'
                                            }`}>
                                            TECHNO
                                        </span>
                                        ...
                                        <span className={`text-sm font-medium ${visibleGenre === 'TRANCE' ? 'text-purple-500' : 'text-gray-400'
                                            }`}>
                                            PSYTRANCE
                                        </span>
                                    </div>
            ...
            <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-16">
                {quartersData.map(({ quarter, techno, trance, visibleGenre, monthRange }) => {
                    ...
                    return (
                        <div key={`quarter-${quarter}`} className="space-y-0 w-11/12 mx-auto rounded-xl md:py-8 bg-slate-900">
                            {/* Header del trimestre */}
                            <div className="text-center">
